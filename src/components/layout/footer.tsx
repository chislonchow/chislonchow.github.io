...
export default function Footer() {
  const [clientDeterminedYear, setClientDeterminedYear] = useState<
    number | null
  >(null);
  ...
  useEffect(() => {
    ...
    const determinedEmailAddress =
      typeof rawEmailEntry === "string" ? rawEmailEntry : "";

    setEmailAddress(determinedEmailAddress);
    setMailtoLink(
      determinedEmailAddress ? `mailto:${determinedEmailAddress}` : "#"
    );
  }, [translations.contactEmailValue]);

  const rawFooterCopyrightText = translations.footerCopyrightText;
  const fullCopyrightText =
    typeof rawFooterCopyrightText === "string" ? rawFooterCopyrightText : "";

  const rawEmailLabel = translations.contactEmailLabel;
  const emailLabelText =
    typeof rawEmailLabel === "object" &&
    rawEmailLabel !== null &&
    typeof rawEmailLabel[language] === "string"
      ? rawEmailLabel[language]
      : typeof rawEmailLabel === "string"
      ? rawEmailLabel
      : "";

  const rawSocialNavLabel = translations.socialMediaNavLabel;
  const socialNavLabelText =
    typeof rawSocialNavLabel === "object" &&
    rawSocialNavLabel !== null &&
    typeof rawSocialNavLabel[language] === "string"
      ? rawSocialNavLabel[language]
      : typeof rawSocialNavLabel === "string"
      ? rawSocialNavLabel
      : "";

  const rawBloggerLabel = translations.socialBloggerLabel;
  const bloggerLabelText =
    typeof rawBloggerLabel === "object" &&
    rawBloggerLabel !== null &&
    typeof rawBloggerLabel[language] === "string"
      ? rawBloggerLabel[language]
      : typeof rawBloggerLabel === "string"
      ? rawBloggerLabel
      : "";

  const rawYoutubeLabel = translations.socialYoutubeLabel;
  const youtubeLabelText =
    typeof rawYoutubeLabel === "object" &&
    rawYoutubeLabel !== null &&
    typeof rawYoutubeLabel[language] === "string"
      ? rawYoutubeLabel[language]
      : typeof rawYoutubeLabel === "string"
      ? rawYoutubeLabel
      : "";

  const rawBlueskyLabel = translations.socialBlueskyLabel;
  const blueskyLabelText =
    typeof rawBlueskyLabel === "object" &&
    rawBlueskyLabel !== null &&
    typeof rawBlueskyLabel[language] === "string"
      ? rawBlueskyLabel[language]
      : typeof rawBlueskyLabel === "string"
      ? rawBlueskyLabel
      : "";

  const rawThreadsLabel = translations.socialThreadsLabel;
  const threadsLabelText =
    typeof rawThreadsLabel === "object" &&
    rawThreadsLabel !== null &&
    typeof rawThreadsLabel[language] === "string"
      ? rawThreadsLabel[language]
      : typeof rawThreadsLabel === "string"
      ? rawThreadsLabel
      : "";

  const rawFooterDisclaimer = translations.footerDisclaimer;
  const footerDisclaimerText =
    typeof rawFooterDisclaimer === "object" &&
    rawFooterDisclaimer !== null &&
    typeof rawFooterDisclaimer[language] === "string"
      ? rawFooterDisclaimer[language]
      : typeof rawFooterDisclaimer === "string"
      ? rawFooterDisclaimer
      : "";
    ...
    <footer className="bg-secondary/50 py-8 mt-auto">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 font-body">
        ...
        <div className="text-center">
          <p className="text-muted-foreground text-sm">{fullCopyrightText}</p>
        </div>
      </div>
    </footer>
  );
