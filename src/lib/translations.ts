...
type TranslationValue = Record<Language, string> | string;
type ProcessedTranslations = Record<string, TranslationValue>;
...
export const getTranslations = (currentYear: number): ProcessedTranslations => {
  ...
  const navigationData = loadYamlFile(translationsDirectory, 'navigation.yaml');
  const contactPopoverData = loadYamlFile(translationsDirectory, 'contact-popover.yaml');
  const accessibilityData = loadYamlFile(translationsDirectory, 'accessibility.yaml');
  ...
  const allTranslations: ProcessedTranslations = {
    ...
    ...processedCategories, // Use the processed categories
    ...(navigationData as ProcessedTranslations),
    ...(contactPopoverData as ProcessedTranslations),
    ...(accessibilityData as ProcessedTranslations),
  };
